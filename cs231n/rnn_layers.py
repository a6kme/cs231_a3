...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    affine_value = x.reshape(x.shape[0], -1).dot(Wx) + prev_h.dot(Wh) + b
    next_h = np.tanh(affine_value)
    cache = (x, prev_h, Wx, Wh, affine_value)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, affine_value = cache

    d_tanh = (1 - np.tanh(affine_value)**2) * dnext_h

    # (N, D)
    dx = d_tanh.dot(Wx.T).reshape(x.shape)

    # (N, H)
    dprev_h = d_tanh.dot(Wh.T)
    dWx = x.reshape(x.shape[0], -1).T.dot(d_tanh)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[0]):
        step_h, step_cache = rnn_step_forward(x[t], prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    # Initialize the last dh_next
    dnext_h = np.zeros((dh.shape[1], dh.shape[2]))

    for t in range(1, dh.shape[0] + 1):
        dh_effective = dh[-t] + dnext_h
        dx_step, dnext_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh_effective, cache[-t])
...
def word_embedding_forward(x, W):
    ...
    # x is N, T. We want (N, T, D) where W is (V, D). Lets get (N, T) vectors from W
    out = W[x]
    cache = (x, W)
...
def word_embedding_backward(dout, cache):
    ...
    np.add.at(dW, x.reshape(-1), dout.reshape(-1, dout.shape[2]))
